 ...
 * <pageview
    ...
    program-id=""
    >
 * @param {Object} ng-model - store a list of data record
 * @param {String} program-id - assign the program id to implement the behavior of pageview
 */
app.directive('pageview', ['$rootScope', 
    ...
    function PageViewConstructor($scope, $element, $attrs) {
		if(Core.GetConfig().debugLog.DirectiveFlow)
            console.log("Pageview.1 - PageViewConstructor()");
        ...
        $scope.Initialize = function(){
			if(Core.GetConfig().debugLog.DirectiveFlow)
				console.log("Pageview.2 - Initialize()");
    	...
    	$scope.GotoLastPageRecord = function(){
            ...
            if($scope.lastPageNum != -1){
//    			 $scope.pageNum = $scope.lastPageNum -1;
                $scope.pageNum++;
    	...
    	$scope.GetNextPageRecords = function(pageNum){
			...
			if($scope.sortedDataSource.length>0){
				keyObj = $scope.sortedDataSource[$scope.sortedDataSource.length-1];
			}
    ...
    function templateUrlFunction(tElement, tAttrs) {
		if(Core.GetConfig().debugLog.DirectiveFlow)
			console.log("Pageview.0 - templateUrlFunction()");
		...
		compile: function compile(tElement, tAttrs, transclude) {
		    return {
		        pre: function preLink(scope, iElement, iAttrs, controller) {
					if(Core.GetConfig().debugLog.DirectiveFlow)
						console.log("Pageview.3 - compile preLink()");
		        },
		        post: function postLink(scope, iElement, iAttrs, controller) {
					if(Core.GetConfig().debugLog.DirectiveFlow)
						console.log("Pageview.4 - compile postLink()");
